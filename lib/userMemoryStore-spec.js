...
describe('userMemoryStore', function() {
  ...
  describe('#resetPassword', function() {
    ...
    it('should error when user not found', function(done) {
      store.resetPassword('invalid_username', 'new', function(err, user) {
        assert(err);
        assert(!user);
        done();
      });
    });
    it('should update the password and reset login attempt count [slow]', function(done) {
      store.resetPassword(userToCreate.username, 'new', function(err) {
        assert(!err);
        store.verifyPassword(userToCreate.username, 'new', function(err, match) {
          assert(!err);
          assert(match);
